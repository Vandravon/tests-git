...
class LoginApp:
    def __init__(self, root):
        ...
        # References to child windows (to avoid opening duplicates)
    ...
    def _open_welcome_window(self):
        ...
        # Login button — go back to the login window
        tk.Button(
            self.welcome_window,
            text="Login",
            width=15,
        ...
        # Configuration button — open the configuration window
        tk.Button(
            self.welcome_window,
            text="Configuration",
            width=15,
    ...
    def _on_welcome_close(self):
        """Handle the welcome window X button: restore the login window before closing."""
    ...
    def _on_open_config(self):
        """Open the configuration window (Welcome window stays open)."""
        # If a config window is already open, just bring it to front
        if self.config_app and self.config_app.window.winfo_exists():
            self.config_app.window.deiconify()
            self.config_app.window.lift()
            return

        self.config_app = ConfigApp(self.welcome_window)
